...
import abi from '../../frontend/src/abi/ChronicleRegistry.json';
...
const aiWallet = new ethers.Wallet(AI_AGENT_PRIVATE_KEY, provider);
const contract = new Contract(CONTRACT_ADDRESS, abi, aiWallet);


// --- AI ANALYSIS FUNCTION ---
async function analyzeTransactionWithAI(txData: any): Promise<string> {
...
app.get('/api/leaderboard', async (req: Request, res: Response) => {
    try {
        ...
        const userAddressesSet = new Set<string>();
...
app.get('/api/chronicles', async (req: Request, res: Response) => {
    try {
        const currentBlock = await provider.getBlockNumber();
        const CHUNK_SIZE = 1000;
        ...
        const chroniclesMap = new Map<string, { txHash: string; narrative: string; requester: string; timestamp: number }>();
...
app.post('/api/check-chronicle-status', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const requestPending = pendingRequester && pendingRequester !== ethers.ZeroAddress;
...
async function waitForPendingRequest(txHash: string, maxAttempts = 60, delayMs = 2000): Promise<boolean> {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            const pendingRequester = await contract.pendingRequests(txHash);
            if (pendingRequester && pendingRequester !== ethers.ZeroAddress) {
